...
apiRouterV3.get(endpoint + 'products', checkToken, function (req, res) {
    ProductService.getAll()
        .then(products => res.status(200).json(products))
        .catch(err => res.status(500).json({ message: 'Erro ao obter os produtos', error: err.message }));
});

apiRouterV3.get(endpoint + 'products/:id', checkToken, function (req, res) {
...
apiRouterV3.post(endpoint + 'products', checkToken, isAdmin, function (req, res) {
...
apiRouterV3.delete(endpoint + 'products/:id', checkToken, isAdmin, function (req, res) {
    ProductService.getById(req.params.id)
        .then(product => {
            if (!product.length)
                return res.status(404).json({ message: 'Produto não encontrado' })
...
apiRouterV3.post(endpoint + 'auth/register', function (req, res) {
...
apiRouterV3.post(endpoint + 'auth/login', function (req, res) {
    UserService.getByLogin(req.body.login)
        .then(users => {
            if (!users.length)
